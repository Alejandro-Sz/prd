...
$("document").ready(function () {
	  ...
	  $('.info').click(function(){
		 ...
		 url1 = urls[0] + "?" + urls[1] + "&" + "filtro=" + $('#filtro').val();
...
<div class="resultados">
	...
	<?php
		...
		$strSQL = "SELECT * FROM adopciones WHERE nombre LIKE '%$filtro%' OR clave LIKE '%$filtro%' OR raza LIKE '%$filtro%' OR sexo LIKE '%$filtro%' OR edad LIKE '%$filtro%' OR comentarios LIKE '%$filtro%' OR custodio LIKE '%$filtro%' OR email LIKE '%$filtro%'";
		//echo $strSQL;
 ...
 <div class='foto' onclick="window.open('detalles.php?clave=<?php echo $objResult['clave'] ?>');" style="cursor: hand; background-image:url('../fotos_adop/<?php echo $objResult['foto1'] ?>'); background-position:center; background-repeat:no-repeat; background-size:contain;">
	   ...
	   ?>
		...
		echo "<input id='filtro' name='filtro' class='txt' type='hidden' value='$filtro' />";

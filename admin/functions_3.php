...
function calendar_html3(){
	...
	<table style="width:200px;text-align:center;border:1px solid #808080;border-bottom:0px;" cellpadding="0" cellspacing="0">
	 <tr>
	  <td colspan="4">
	  	<select id="calendar_mes" onchange="update_calendar3()">
		 <?php
		 $mes_numero=1;
		 while($mes_numero<=12){
		 	if($mes_numero==$mes){
	  ...
	  <td colspan="3">
	  	<select style="width:70px;" id="calendar_anio" onchange="update_calendar3()">
		 <?php
		 // años a mostrar
	...
	<?php calendar3($mes,$anio) ?>
	</div>
	<?php
}


function calendar3($mes,$anio){
	$dia=1;
	...
	$ultimo_dia=ultimoDia3($mes,$anio);
	...
	while($j<$total_dias){
		...
		while($i<7){
			if($j<=$numero_primer_dia){
				...
				//echo " <td><a style=\"display:block;cursor:pointer;\" onclick=\"set_date3('".$diames_con_cero."-".$mes."-".$anio."')\">".$diames."</a></td> \n";
				echo " <td><a style=\"display:block;cursor:pointer;\" onclick=\"set_date3('".$anio."-".$mes."-".$diames_con_cero."')\">".$diames."</a></td> \n";
				$diames++;
			}
			$i++;
			$j++;
